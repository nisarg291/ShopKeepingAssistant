...
def parse_voice_transcript(transcript):
    items = []
    # Split transcript by 'and' or commas to handle multiple items
    parts = re.split(r'\s*(?:and|;)\s*', transcript.lower().strip(), flags=re.IGNORECASE)
    pattern = r'add\s+(\d+)\s+([^\s].*?)\s+at\s+(\d+\.?\d*)\s+dollars'
    ...
    if request.method == 'POST' and 'voice_form' in request.form:
        ...
        try:
            if voice_form.validate_on_submit():
                ...
                if not items:
                    flash('Could not parse any items. Use format: "Add <quantity> <item name> at <price> dollars" and separate items with "and" or ";".', 'error')
    ...
    if request.method == 'POST':
        ...
        try:
            if edit_form.validate_on_submit():
                ...
                flash(f'Form validation failed: {edit_form.errors}', 'error')
        except CSRFError as e:
